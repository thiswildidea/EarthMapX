...
import { captcha,signIn,getInfo,getRouters } from '/@/api/login/index';
...
const onSignIn = async () => {
    ...
    let usererinfo = await getInfo();
    let routers = await getRouters();

	Session.set('menus', routers.menus);
	let perms = usererinfo.permissions
	perms.push('base')
	// 用户信息
	const userInfos = {
		username: usererinfo.user.userName,
		userId: usererinfo.user.userId,
		roleId: usererinfo.user.roles[0].roleId,
		deptId: usererinfo.user.dept.deptId,
		postId: usererinfo.user.postId,
		// 头像
		photo: usererinfo.user.avatar || letterAvatar(usererinfo.user.userName),
