    ...
    const openDialog = (row: any) => {
      if (row && row.userId && row.userId != undefined && row.userId != 0) {
        getUser(row.userId).then((response: any) => {
          ...
          state.postIds = response.postIds.map((item: string)=>{
            return Number(item)
          });
          state.roleIds = response.roleIds.map((item: string)=>{
            return Number(item)
          });
    ...
    const onSubmit = () => {
      ...
      formWrap.validate((valid: boolean) => {
        if (valid) {
          ...
          state.ruleForm.postIds = state.postIds
          state.ruleForm.roleIds = state.roleIds
