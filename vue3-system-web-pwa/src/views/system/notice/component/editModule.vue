      ...
      >
        ...
        <el-col :span="24">
          <el-form-item label="通知内容" prop="noticeContent">
            <Editor v-model="ruleForm.noticeContent"></Editor>
          </el-form-item>
        </el-col>
      </el-form>
      <template #footer>
  ...
  setup() {
    ...
    const openDialog = (row: any) => {
      ...
      if (row.noticeId && row.noticeId != undefined && row.noticeId != "") {
        ...
        state.ruleForm.noticeTitle = row.noticeTitle;
        state.ruleForm.noticeType = row.noticeType;
        state.ruleForm.deptId = row.deptId;
        state.ruleForm.noticeContent = row.noticeContent;
      } else {
        initForm();
        state.ruleForm.noticeContent = "";
      }
      ...
      treeselect().then((response: any) => {
        state.deptOptions = [];
        const dept = { id: 0, label: '全部部门', children: [] }
